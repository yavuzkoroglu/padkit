...
On the other hand, **over-standardization** can harm the utility of a programming language; taking away the developer's ability to implement a portable program by drowning the developer. ISO/IEC 9899:TC3 [^2], or C99 in short, is the standard I developed PADKIT on. However, C99 is not the final standard for a command-line program developer. The POSIX.1-XXXX family of standards build on top of C standards. If standardization is so beneficial, why not adopt a POSIX standard?
...
> [!WARNING]
> What about the fact that many operating systems only partially conform to POSIX?

> [!WARNING]
> What about the fact that different operating systems conform to different POSIX standards?

In short, reasonable people will agree that there has to be a limit to standardization, especially considering the development of general-purpose programs. Then, the question is about where the limit should be. I decided to stick to the C99 standard only to keep the number of constraints on PADKIT concise, while keeping PADKIT compatible with all the popular operating systems; Linux, MacOS, and Windows.
